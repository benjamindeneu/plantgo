...
async function validatePicture({ species = null, file }) {
  try {
    ...
    let total_points, points, introText, pointsBreakdown;
    
    // Determine points based on species-specific logic if species is provided
    if (species) {
      ...
      introText = `<p><strong>Species Validation in Progress...</strong></p>`;
      pointsBreakdown = `<h2>Identification Results</h2>`;
      ...
      // General picture validation
      const result = await getPoints(lat, lon, bestMatch);
      total_points = result.total_points;
      points = result.points;
      introText = `<p><strong>General Plant Picture Validation in Progress...</strong></p>`;
    ...
    // Determine mission level (common logic)
    ...
    // Store data in sessionStorage and redirect to the validation page
    ...
    sessionStorage.setItem('introText', `<p style="color: red;">Error validating photo${species ? ' for ' + species.name : ''}</p>`);
...
async function validateSpeciesPicture(species, file) {
  validatePicture({ species: selectedSpecies, file });
}
